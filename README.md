...
Only STL and [Boost](https://www.boost.org/) C++ libraries are used. Please check files ```main.cpp``` and ```mobility.h``` for all required libraries.

Tested on these C++ compilers: gcc 5.4.0, gcc 11.4.0, gcc 13.3.0.

Compiled & tested at 64-bit environments: 

(a) Debian GNU/Linux 7.8 (wheezy); 

(b) Ubuntu 22.04.5 LTS; and 

(c) Cygwin terminal 2.11.1 in Windows 10.
...
Note that (in both options) ```annotate``` is the name of the resulting executable software.
...
- _MAX_RATE_OF_CHANGE_: if rate of change in speed is above this value (in knots per hour), the location may be _NOISE_.

- _MAX_RATE_OF_TURN_: if rate of turn is above this value in degrees (azimuth) per second, the location may be _NOISE_.

- _DISTANCE_THRESHOLD_: under this distance (in meters) from its previous location, the vessel may be _STOPPED_.
...
- ```[input-file]```: Path to the input data (a ASCII space delimited file). The contents of the file are used to simulate a positional data stream based on the incoming AIS timestamped locations from vessels.

- ```[id-attr]```: An integer specifying which attribute contains the vessel identifiers in input data; give a negative integer (e.g., -1) if NO vessel identifiers are included in the input data.

- ```[timestamp-attr]```: An integer specifying which attribute contains the timestamp values in the input data (e.g., give 3, if timestamps are in the 3rd field in input file).

- ```[settings-json]```: Path to JSON file with configuration settings per vessel type. 

- ```[vessel-info-file]```: Path to a CSV file with vessel information. This semicolon-separated (';') file includes the general AIS type (e.g., passenger, cargo, fishing) of each vessel in the input data.
...
- The input data concerns a _SINGLE_ vessel and its identifier is not included in the input:
```
...
- The input data concerns _MULTIPLE_ vessels (identifiers must be included) and specify ```true``` to only fetch annotated locations in the output:
```
./annotate ./input/vessels.csv 1 4 ./settings/params.json ./settings/vessel_info.csv ./output/vessels_annotated.csv true
```
...
This schema may also be applied when input data concerns a _SINGLE_ vessel.

- In case of a _SINGLE_ vessel, the input may alternatively be given according to the following schema:

```
< lon lat t >
```

but then the _Default_ parametrization will be applied, ignoring the vessel's type.

The attributes in the input have the following significance:
...
This is the record schema (as _space separated_ values) in the output file: 
